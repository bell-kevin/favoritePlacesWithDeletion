...
Modify the project so that you can delete a favorite place. Begin with at least 2 favorite places -- take a screenshot showing the AllPlaces screen showing the places. Remove the first one from the database when the delete action is selected by the user. Take a screenshot showing the AllPlaces screen which shows the first place is no longer on the list. Note: It must be the first place, not the second one, that is deleted.
